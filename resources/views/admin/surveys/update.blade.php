    ...
    <div class="bg-white p-6 mt-12 shadow-md rounded-lg">
        ...
        <form action="{{ route('surveys.update', $survey->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="titre" id="titre" value="{{ $survey->titre }}" class="w-full p-2 border rounded">
            </div>
    
            <div class="mb-4">
                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                <textarea name="description" id="description" class="w-full p-2 border rounded">{{ $survey->description }}</textarea>
            </div>
    
            <div class="mb-4">
                <label for="recompense_en_points" class="block text-sm font-medium text-gray-700">Points de Récompense</label>
                <input type="number" name="recompense_en_points" id="recompense_en_points" value="{{ $survey->recompense_en_points }}" class="w-full p-2 border rounded">
            </div>
